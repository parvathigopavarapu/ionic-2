...
import { CreateItemExtraOptionsPage, EditItemExtraOptionsPage } from "../index";

@Component({
    selector: 'page-extraitem',
    templateUrl: 'listitemextraoption.html'
})
export class ItemExtraOptionsListPage {
    number = [];
    food = [];
    detail = [];
    price = [];
    ...
    createItemExtra() {
        let name = this.name;
        this.navCtrl.push(CreateItemExtraOptionsPage,{name})
    }
    editfields() {
         let name = this.name;
        this.navCtrl.push(EditItemExtraOptionsPage,{name})
    }
}